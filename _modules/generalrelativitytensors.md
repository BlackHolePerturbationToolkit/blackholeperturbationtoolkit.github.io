---
name: "GeneralRelativityTensors"
summary: "Coordinate-based tensor calculations with a focus on black holes"
lang: "Mathematica"
domain: "tensors"
status: "stable"
repo: "https://github.com/BlackHolePerturbationToolkit/GeneralRelativityTensors"
docs: "https://bhptoolkit.org/GeneralRelativityTensors/"
---

<!-- Scaffolded from _data/tools.yml. Replace the TODO sections with real
     documentation, then delete the stub notice below. -->

<div class="callout" markdown="1">
**Stub page.** Full documentation for GeneralRelativityTensors is in progress. For now, see the [source repository](https://github.com/BlackHolePerturbationToolkit/GeneralRelativityTensors) and the [package page](https://bhptoolkit.org/GeneralRelativityTensors/).
</div>

## Overview

Coordinate-based tensor calculations with a focus on black holes.

<!-- TODO: expand — what it computes, key features, and related packages. -->

## Installation

Clone the repository, add it to your Mathematica ``$Path``, and load it with ``Needs``. See the [repository README](https://github.com/BlackHolePerturbationToolkit/GeneralRelativityTensors) for details.

## Usage

<!-- TODO: add a minimal, runnable example. -->

```mathematica
(* example coming soon — see the repository for current usage *)
```

## Examples

<!-- TODO: link to worked examples or notebooks. -->
See the [repository](https://github.com/BlackHolePerturbationToolkit/GeneralRelativityTensors) for examples.
