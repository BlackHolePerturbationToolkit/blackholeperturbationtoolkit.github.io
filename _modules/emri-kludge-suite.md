---
name: "EMRI Kludge Suite"
summary: "Kludge waveforms for generic extreme mass-ratio inspirals into Kerr"
lang: "C/C++"
domain: "waveforms"
status: "deprecated"
repo: "https://github.com/alvincjk/EMRI_Kludge_Suite"
---

<!-- Scaffolded from _data/tools.yml. Replace the TODO sections with real
     documentation, then delete the stub notice below. -->

<div class="callout" markdown="1">
**Stub page.** Full documentation for EMRI Kludge Suite is in progress. For now, see the [source repository](https://github.com/alvincjk/EMRI_Kludge_Suite).
</div>

## Overview

Kludge waveforms for generic extreme mass-ratio inspirals into Kerr.

<!-- TODO: expand — what it computes, key features, and related packages. -->

## Installation

Build from source — clone the repository and follow the dependency and build instructions in its README:

```bash
git clone https://github.com/alvincjk/EMRI_Kludge_Suite
```

## Usage

<!-- TODO: add a minimal, runnable example. -->

```cpp
// example coming soon — see the repository for current usage
```

## Examples

<!-- TODO: link to worked examples or notebooks. -->
See the [repository](https://github.com/alvincjk/EMRI_Kludge_Suite) for examples.
