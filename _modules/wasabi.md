---
name: "WaSABI"
summary: "Waveforms from self-force theory and SF-PN hybrids"
lang: "Mathematica"
domain: "self-force"
status: "beta"
repo: "https://github.com/BlackHolePerturbationToolkit/WaSABI"
docs: "https://bhptoolkit.org/WaSABI/"
---

<!-- Scaffolded from _data/tools.yml. Replace the TODO sections with real
     documentation, then delete the stub notice below. -->

<div class="callout" markdown="1">
**Stub page.** Full documentation for WaSABI is in progress. For now, see the [source repository](https://github.com/BlackHolePerturbationToolkit/WaSABI) and the [package page](https://bhptoolkit.org/WaSABI/).
</div>

## Overview

Waveforms from self-force theory and SF-PN hybrids.

<!-- TODO: expand — what it computes, key features, and related packages. -->

## Installation

Clone the repository, add it to your Mathematica ``$Path``, and load it with ``Needs``. See the [repository README](https://github.com/BlackHolePerturbationToolkit/WaSABI) for details.

## Usage

<!-- TODO: add a minimal, runnable example. -->

```mathematica
(* example coming soon — see the repository for current usage *)
```

## Examples

<!-- TODO: link to worked examples or notebooks. -->
See the [repository](https://github.com/BlackHolePerturbationToolkit/WaSABI) for examples.
